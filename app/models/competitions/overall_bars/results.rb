    ...
    module Results
      ...
      def after_create_competition_results_for(race)
        race.results.each do |result|
          scores = result.scores.sort_by(&:points).reverse
          scores = remove_duplicate_discipline_results(scores)

          if scores.size > 5
            lowest_scores = scores[5, scores.size - 5]
      ...
      def remove_duplicate_discipline_results(scores)
        ...
        scores.each do |score|
          ...
          if race.category == cat_5 && cat_4_disciplines.include?(race.discipline)
            logger.debug(
              "Cat 4 result already exists: #{race.discipline} results for #{score.source_result.person}: #{race.category.name}"
            ) if logger.debug?
            scores_to_delete << score
          end
        end

        scores_to_delete.each do |s|
          scores.delete s
          s.destroy
        end
        ...
        scores.each do |score|
          if disciplines.include?(score.source_discipline)
            logger.debug(
              "Multiple #{score.source_discipline} results for #{score.source_result.person}: #{score.source_result.race.category.name}"
            ) if logger.debug?
            scores_to_delete << score
          else
