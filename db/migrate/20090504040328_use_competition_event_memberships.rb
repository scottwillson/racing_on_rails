class UseCompetitionEventMemberships < ActiveRecord::Migration
  def self.up
    Event.find.all( :conditions => "cat4_womens_race_series_id is not null").each do |event|
      ...
      unless CompetitionEventMembership.first(:conditions => { :event_id => event.id, :competition_id => cat4_womens_race_series_id })
    ...
    Event.find.all( :conditions => "oregon_cup_id is not null").each do |event|
      ...
      unless CompetitionEventMembership.first(:conditions => { :event_id => event.id, :competition_id => oregon_cup_id })
