...
namespace :racing_on_rails do
  ...
  task :deploy_release do
    puts(`scp pkg/racing_on_rails-0.0.5.zip butlerpress.com:public_html/racing_on_rails/downloads/racing_on_rails.zip`)
    puts(`scp pkg/racing_on_rails-0.0.5.tar.gz butlerpress.com:public_html/racing_on_rails/downloads/racing_on_rails.tar.gz`)
  end
  ...
  task :acceptence => [:create_app] do
    ...
    run_acceptence_user(File.expand_path('~/racing_on_rails-0.0.5'))
    puts('customize')
    customize
    puts('acceptence_developer')
    run_acceptence_developer(File.expand_path('~/racing_on_rails-0.0.5'))
  end

  task :create_app do
    app_path = File.expand_path('~/racing_on_rails-0.0.5')
    rm_rf app_path
  
    puts(`tar zxf pkg/racing_on_rails-0.0.5.tar.gz -C ~`)
  ...
  task :customize do
    ...
    path = File.expand_path('~/racing_on_rails-0.0.5')
  ...
  def customize(fixture_path)
    mkpath(File.dirname(File.expand_path("~/racing_on_rails-0.0.5/#{fixture_path}")))
    cp(File.expand_path("/test/fixtures/#{fixture_path}"),
       File.expand_path("~/racing_on_rails-0.0.5/#{fixture_path}"))
  end
  ...
  Rake::PackageTask.new("racing_on_rails", "0.0.5") do |p|
    p.need_tar_gz = true
    p.need_zip = true
