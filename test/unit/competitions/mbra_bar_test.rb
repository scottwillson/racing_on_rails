...
class MbraBarTest < ActiveSupport::TestCase
  ...
  def test_calculate
    ...
    MbraBar.calculate!(2008)
    assert_equal(7, MbraBar.count(:conditions => ['date = ?', Date.new(2008)]), "Bar events after calculate!")
    assert_equal(original_results_count + 4 + 3 + 2, Result.count, "Total count of results in DB")
    ...
    assert_equal(0, women_road_bar.results.size, "Senior Women Road BAR results")
  end
