...
module Posts
  class MigrationTest < ActiveSupport::TestCase
    ...
    test "reposition!" do
      mailing_list = FactoryGirl.create(:mailing_list)
      last_original = FactoryGirl.create(:post, :mailing_list => mailing_list, :subject => "For Sale: Trek Madrone", :date => 2.day.ago, :position => 0)
      first_original = FactoryGirl.create(:post, :mailing_list => mailing_list, :subject => "Autographed TDF Jersey", :date => 4.days.ago, :position => 2)
      reply_to_last_original = FactoryGirl.create(:post, :mailing_list => mailing_list, :subject => "re: For Sale: Trek Madrone", :date => 1.days.ago, :position => 3)
